...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_viptienda:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_viptienda:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
...
RegisterNetEvent('esx_viptienda:sendCategories')
AddEventHandler('esx_viptienda:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_viptienda:sendVehicles')
AddEventHandler('esx_viptienda:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_viptienda:getCommercialVehicles', function(vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_viptienda:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('esx_viptienda: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_viptienda:buyVehicleSociety', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_viptienda:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
											...
											if Config.EnableOwnedVehicles then
												TriggerServerEvent('esx_viptienda:setVehicleOwned', vehicleProps)
											end
								...
								ESX.TriggerServerCallback('esx_viptienda:buyVehicleSociety', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
											...
											vehicleProps.plate = newPlate
											SetVehicleNumberPlateText(vehicle, newPlate)
											TriggerServerEvent('esx_viptienda:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
											ESX.ShowNotification(_U('vehicle_purchased'))
										end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('esx_viptienda:sellVehicle', model)
				TriggerServerEvent('esx_viptienda:addToList', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('esx_viptienda:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function(xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('esx_viptienda:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						TriggerServerEvent('esx_viptienda:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_viptienda:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		elseif data.current.value == 'sold_vehicles' then
			...
			ESX.TriggerServerCallback('esx_viptienda:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_viptienda:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_viptienda:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_viptienda:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_viptienda:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('esx_viptienda:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_viptienda:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_viptienda:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_viptienda:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_viptienda:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_viptienda:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
